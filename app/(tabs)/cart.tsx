...
import { Image, ScrollView, Text, View, TouchableOpacity, useColorScheme, Modal, useWindowDimensions } from 'react-native';
...
export default function CartScreen() {
  ...
  // 1. Get the exact screen height dynamically
  ...
  const handleUpdateQuantity = (id: string, type: 'increase' | 'decrease') => {
    setCartItems(prev => prev.map(item => {
      if (item.id === id) {
        return { ...item, quantity: Math.max(1, type === 'increase' ? item.quantity + 1 : item.quantity - 1) };
      }
      return item;
    }));
  };

  // Calculates total only for checked items
  ...
  return (
    <View className="flex-1 bg-white dark:bg-[#121212]">
        ...
        { cartItems.length === 0 ? (
          ...
          {cartItems.map((item) => (
            <View key={item.id} className='items-center flex justify-center w-full'>
              <View className='flex-row bg-white dark:bg-[#1E1E1E] my-3 w-[92%] p-3.5 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm'>
                ...
                <View className='flex-1 flex-col py-1.5 justify-between'>
                  ...
                  <View className='flex-row items-center space-x-2 mt-2'>
                    <TouchableOpacity onPress={() => handleUpdateQuantity(item.id, 'decrease')} className='w-7 h-7 bg-[#0056D2] rounded-md justify-center items-center'>
                    ...
                    <TouchableOpacity onPress={() => handleUpdateQuantity(item.id, 'increase')} className='w-7 h-7 bg-[#0056D2] rounded-md justify-center items-center'>
      ...
      >
        {/* 2. Used absolute positioning with flex-end to ensure strict bottom placement */}
        <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(0,0,0,0.4)' }}>
          <View 
            // 3. Added dynamic maxHeight and paddingBottom to avoid overlapping with iOS Home Bar
            style={{ maxHeight: screenHeight * 0.9, paddingBottom: 40 }}
            className="bg-white dark:bg-[#1E1E1E] rounded-t-[32px] px-6 pt-6 w-full shadow-lg items-center relative"
          >
            
            <View className="flex-row justify-center items-center w-full mb-8 relative">
                <Text className="text-lg font-semibold text-[#333333] dark:text-white">Select Printer</Text>
                <TouchableOpacity onPress={() => setModalVisible(false)} className="absolute right-0">
                    <Ionicons name="close-outline" size={24} color={isDark ? "#FFF" : "#333"} />
                </TouchableOpacity>
            </View>

            <View className="w-24 h-24 justify-center items-center mb-6">
                <Image source={require('@/assets/images/printer-icon.png')} resizeMode='contain' style={{ width: '100%', height: '100%' }} />
            </View>

            <Text className="text-[#828282] w-76 mt-6 text-center tracking-wider dark:text-gray-400 text-md leading-relaxed px-4 mb-8">
              Kindly select, message and negotiate with any Printer on our platform before we redirect you to your check out page. Thanks
            </Text>

            <TouchableOpacity 
              onPress={() => {
                  setModalVisible(false);
                  router.push('/select-printer'); 
              }}
              className="w-full bg-[#3B2D85] rounded-full py-4 items-center justify-center mb-4"
            >
                <Text className="text-white w-full text-center font-bold text-base">Select Printer</Text>
            </TouchableOpacity>

            <View className="w-12 h-1 bg-gray-300 dark:bg-gray-600 rounded-full mt-2" />
