...
import { ActivityIndicator, Alert, FlatList, Image, SafeAreaView, Text, TouchableOpacity, View, useColorScheme } from 'react-native';
      ...
      <View className="flex-1 px-6 pt-12">
        <View className="mb-6 flex-row items-center justify-between py-4">
          ...
          <TouchableOpacity onPress={clearFavorites} disabled={!designs.length}>
            <Text className={`text-base font-semibold ${designs.length ? 'text-[#EB5757]' : 'text-[#EB5757]/40'}`}>Clear</Text>
          </TouchableOpacity>
        </View>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            contentContainerStyle={{ paddingBottom: 28, flexGrow: designs.length ? 0 : 1 }}
            renderItem={({ item }) => {
              const artistName = item.designerName || `${item.profile.firstName} ${item.profile.lastName}`.trim() || item.profile.username;
              const imageUri = item.imagePath?.startsWith('http')
                ? item.imagePath
                : item.imagePath
                  ? `https://berrystamp-backend-dev-4cn29.ondigitalocean.app/${item.imagePath.replace(/^\/+/, '')}`
                  : '';
              ...
              return (
                <View className="mb-3 flex-row rounded-2xl border border-gray-100 bg-white p-3 dark:border-gray-800 dark:bg-[#1E1E1E]">
                  <View className="mr-3 h-24 w-24 overflow-hidden rounded-xl bg-gray-100 dark:bg-gray-800">
                    {imageUri ? <Image source={{ uri: imageUri }} className="h-full w-full" resizeMode="cover" /> : null}
                  </View>

                  <View className="flex-1 justify-between">
                    <View>
                      <Text numberOfLines={1} className="text-base font-semibold text-[#1E1B2B] dark:text-white">{item.title}</Text>
                      <Text numberOfLines={1} className="mt-1 text-sm text-[#8F889D] dark:text-gray-400">By {artistName}</Text>
                    </View>

                    <View className="mt-3 flex-row items-end justify-between">
                      <Text className="text-lg font-bold text-[#1D1A2B] dark:text-white">{formatNaira(item.amount || 0)}</Text>
                      <View className="items-end">
                        ...
                        <TouchableOpacity onPress={() => router.push({ pathname: '/product', params: { designId: String(item.id) } })}>
                          <Text className="mt-3 text-sm font-semibold text-[#3F2FA0]">See details</Text>
                        </TouchableOpacity>
                      </View>
                    </View>
                  </View>
            ...
            ListEmptyComponent={
              <View className="flex-1 items-center justify-center px-6">
                <Ionicons name="heart-outline" size={84} color={isDark ? '#7A7A7A' : '#B8B6BF'} />
                <Text className="mt-4 text-center text-xl font-semibold text-[#2B2833] dark:text-white">Favorite is empty</Text>
                <Text className="mt-3 text-center text-sm leading-6 text-[#8A8694] dark:text-gray-400">
                  You have not yet added any item to your favorite list. Explore beautiful designs and save the ones you love.
                </Text>
              </View>
            }
          />
