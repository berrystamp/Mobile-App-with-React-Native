        ...
        <View className="px-4 pb-4 pt-12">
          ...
          <View className="rounded-[28px] bg-[#4A34A5] px-5 py-5 dark:bg-[#362774]">
            ...
            <View className="mt-5 flex-row flex-wrap">
              ...
              <StatusPill label={items.length ? `${items.length} item${items.length > 1 ? 's' : ''}` : 'Items'} active={Boolean(items.length)} />
            </View>
          </View>
          ...
          <View className="mt-6">
            ...
            <View className="mt-5 rounded-[24px] bg-white px-4 py-4 shadow-sm dark:bg-[#1E1E1E]">
              ...
              {items.length ? (
                ...
                <TouchableOpacity
                  onPress={() => router.push({ pathname: '/SelectItemsScreen', params: { draft: encoded } })}
            ...
            <View className="mt-5 rounded-[24px] bg-white px-4 py-4 shadow-sm dark:bg-[#1E1E1E]">
              ...
              <SummaryRow label="Print items" value={items.length ? items.join(', ') : 'No item selected'} isLast />
            </View>
          </View>
        </View>
      </ScrollView>
  ...
  return (
    <TouchableOpacity onPress={onPress} className="mb-4 rounded-[24px] bg-white px-4 py-4 shadow-sm dark:bg-[#1E1E1E]">
      ...
      <View className="flex-row items-center justify-between rounded-xl border border-[#E7E3F2] px-4 py-4 dark:border-[#33333A]">
        <Text className={`flex-1 text-[14px] ${isPlaceholder ? 'text-[#A19BAF] dark:text-[#7D7D88]' : 'text-[#2E2939] dark:text-white'}`}>
          {value}
        </Text>
...
function SummaryRow({
  ...
  return (
    <View className={`flex-row items-start justify-between py-3 ${isLast ? '' : 'border-b border-[#F0EBF7] dark:border-[#31313A]'}`}>
