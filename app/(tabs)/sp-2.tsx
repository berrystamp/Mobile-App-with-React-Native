...
export default function SponsoredPaymentStep2() {
  const router = useRouter();
  const { flow } = useLocalSearchParams<{ flow?: string }>();
  const draft = useMemo(() => decodeSponsoredDraft(flow), [flow]);
  const [selected, setSelected] = useState(draft.designTheme);
  ...
  return (
    <View className="flex-1 bg-[#F7F6FB] px-5 pt-12 dark:bg-[#121212]">
      <View className="mb-6 flex-row items-center justify-between">
        ...
        <Text className="text-base font-semibold text-[#2D273A] dark:text-white">Sponsored Payment (2/11)</Text>
        <View className="w-10" />
      </View>

      <Text className="text-2xl font-bold text-[#2D273A] dark:text-white">Select your design theme</Text>
          ...
          {DEFAULT_DESIGN_THEMES.map((theme) => {
            ...
            return (
              <TouchableOpacity
                ...
                className={`mb-2 flex-row items-center justify-between rounded-2xl px-4 py-4 ${active ? 'bg-[#EEE9FF]' : 'bg-transparent'}`}>
                <Text className={`text-base font-semibold ${active ? 'text-[#4A34A5]' : 'text-[#312B3F] dark:text-white'}`}>{theme}</Text>
      ...
      <View className="absolute bottom-0 left-0 right-0 border-t border-[#ECE8F4] bg-[#F7F6FB] px-5 pb-8 pt-4 dark:border-[#232327] dark:bg-[#121212]">
        <TouchableOpacity
          disabled={!selected}
          onPress={() => router.push({ pathname: '/sp-3', params: { flow: encodeSponsoredDraft({ ...draft, designTheme: selected }) } })}
