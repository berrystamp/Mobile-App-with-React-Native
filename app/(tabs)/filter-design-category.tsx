...
export default function FilterProductCategoryScreen() {
  ...
  useEffect(() => {
    const loadState = async () => {
      ...
      setSelectedItems(filters.designCategories);
      setItems(Array.from(new Set(designs.flatMap((design) => design.categories || []).filter(Boolean))));
    };

    loadState().catch((error) => console.error('Failed to load design categories', error));
  }, []);
  ...
  const handleApply = async () => {
    ...
    await setSearchFilters({ ...filters, designCategories: selectedItems });
    router.back();
  };
  ...
  const handleClear = async () => {
    setSelectedItems([]);
    const filters = await getSearchFilters();
    await setSearchFilters({ ...filters, designCategories: defaultSearchFilters.designCategories });
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}> 
      <View style={styles.topBar}>
        ...
        <Text style={[styles.topTitle, { color: theme.text }]}>Design Category</Text>
